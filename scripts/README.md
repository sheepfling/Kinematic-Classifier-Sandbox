...
- Product 4 lane-isolated evaluation matrix: `PYTHONPATH=src python3 scripts/audit/evaluate_product4_lane_matrix.py --snapshot /external/product4-six-lane/snapshot.json --assignments /external/product4-six-lane/assignments.json --output /external/product4-six-lane/lane-matrix.json`; add `--grouping-namespace physical_platform` to evaluate a declared task-scoped grouping policy.
...
- Product 4 analysis test surfaces: `PYTHONPATH=src python3 -m pytest -q -m product4_source_audit`, `product4_kinematic_analysis`, or `product4_classifier_ladder`; use the narrow marker that matches the claim.
- Build the external task-scoped LAND classifier cohort: `PYTHONPATH=src python3 scripts/run/build_tgsim_prepared_classifier_snapshot.py --source /external/tgsim_balanced.csv --snapshot-root /external/product4-land-prepared --snapshot-id product4-land-prepared-platform-class-v1-2026-08-25 --source-dataset-id fhwa_tgsim_foggy_bottom_balanced_vehicle_cohort_v1 --source-artifact-id fhwa_tgsim_foggy_bottom_balanced_vehicle_cohort_v1_csv`
- Run the bounded LAND Product 2 bridge: `PYTHONPATH=src python3 scripts/audit/run_product2_real_world_bridge.py --snapshot /external/product4-land-prepared/snapshot.json --analysis-manifest /external/product4-land-prepared/classifier-ladder.json --assignments /external/product4-land-prepared/assignments.json --output-dir /external/product4-land-bridge --expected-lane land_surface --target-label-namespace platform_class --pair-id passenger_car_vs_truck --class-a passenger_car --class-b truck --grouping-namespace physical_platform --max-episodes-per-group 1`
- Build the external task-scoped SEA-SURF route cohort: `PYTHONPATH=src python3 scripts/run/build_cmre_prepared_classifier_snapshot.py --tracklets /secure/tracklets.csv --nomenclature /secure/nomen.csv --identity-key /secure/cmre-identity.key --snapshot-root /external/product4-sea-surface-prepared --snapshot-id product4-sea-surface-prepared-route-r06-r14-v1 --route-a R_06 --route-b R_14`
- Run the bounded SEA-SURF Product 2 bridge with `--target-label-namespace route --pair-id route_r06_vs_r14 --class-a R_06 --class-b R_14`; route association is not a vessel-family label.
