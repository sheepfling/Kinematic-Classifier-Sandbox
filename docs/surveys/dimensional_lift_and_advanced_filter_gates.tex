...
linear-Gaussian algorithm whose justification is still evidence-gated. The
recursion written below follows the standard multiple-model filtering
construction rather than a repo-specific variant \cite{barshalom2001}.
...
The current repo now has concrete PF and RBPF implementations, and both are now
also exposed through the shared classifier surface even though they remain
decision-gated rather than universal defaults. The main code surfaces are:
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \texttt{advanced\_filters/shared\_classifier\_methods.py}: shared
    classifier adapters that lift PF and RBPF into the generic registry,
    \item \texttt{advanced\_filters/rbpf.py}: sampled discrete latent update plus
    conditional Kalman subfilter,
    \item \texttt{advanced\_filters/ou\_witness.py}: concrete mean-reverting
    Ornstein-Uhlenbeck-style witness built on the PF branch,
