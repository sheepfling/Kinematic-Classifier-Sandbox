...
class LeakageConstraintSpec:
    variable_name: str
    max_delta_ratio: float


@dataclass(frozen=True, slots=True)
class CorpusObjectiveSpec:
    objective_id: str
    description: str
    ...
    target_environment_regimes: tuple[str, ...] = ()
    leakage_constraints: tuple[LeakageConstraintSpec, ...] = ()
    backend_constraints: tuple[str, ...] = ()
    runtime_budget: dict[str, int] = field(default_factory=dict)


@dataclass(frozen=True, slots=True)
class CorpusObjectiveArtifacts:
    run_dir: Path
    schema_path: Path
...
def _schema() -> dict[str, Any]:
    return {
        ...
        "title": "CorpusObjectiveSpec",
        "type": "object",
        "required": ["objective_id", "description", "target_difficulty", "runtime_budget"],
        "properties": {
            ...
            "target_feature_excitation": {"type": "object"},
            "target_difficulty": {"type": "string"},
            "target_posterior_entropy": {"type": ["string", "null"]},
            "target_environment_regimes": {"type": "array", "items": {"type": "string"}},
            "leakage_constraints": {
                "type": "array",
                "items": {
                    "type": "object",
                    "required": ["variable_name", "max_delta_ratio"],
                    "properties": {
                        "variable_name": {"type": "string"},
                        "max_delta_ratio": {"type": "number"},
                    },
                },
            },
            "backend_constraints": {"type": "array", "items": {"type": "string"}},
...
def default_corpus_objectives() -> tuple[CorpusObjectiveSpec, ...]:
    return (
        CorpusObjectiveSpec(
            ...
            leakage_constraints=(
                LeakageConstraintSpec("duration", 0.20),
                LeakageConstraintSpec("measurement_std", 0.20),
            ),
        ...
        CorpusObjectiveSpec(
            ...
            leakage_constraints=(LeakageConstraintSpec("num_samples", 0.25),),
        ...
        CorpusObjectiveSpec(
            ...
            leakage_constraints=(
                LeakageConstraintSpec("density_scale", 0.15),
                LeakageConstraintSpec("wind_bias", 0.15),
            ),
...
def analyze_corpus_objectives() -> CorpusObjectiveResult:
    ...
    report_markdown = "\n".join(
        [
            ...
            "| --- | --- | --- | --- | --- | --- | --- |",
            *[
                f"| `{objective.objective_id}` | `{objective.target_class or ''}` | "
                f"`{' vs '.join(objective.target_class_pair) if objective.target_class_pair else ''}` | "
                f"`{len(objective.target_feature_excitation)}` | `{len(objective.target_environment_regimes)}` | "
                f"`{len(objective.leakage_constraints)}` | `{objective.runtime_budget}` |"
                for objective in objectives
            ],
            "",
            "## Notes",
...
def _objective_to_dict(objective: CorpusObjectiveSpec) -> dict[str, Any]:
    ...
    payload["leakage_constraints"] = [asdict(item) for item in objective.leakage_constraints]
    return payload
...
def write_corpus_objective_artifacts(
    ...
    *,
    ...
    schema_path.write_text(json.dumps(payload.schema, indent=2), encoding="utf-8")
