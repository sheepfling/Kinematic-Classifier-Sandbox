...
from kinematic_classifier_sandbox.utils.stats import histogram_overlap as _histogram_overlap, js_divergence as _js_divergence
...
class FeatureRow(FeatureValueMappingMixin):
    trajectory_id: str
    tier: str
    scenario_id: str
    true_class: str
...
@dataclass(frozen=True, slots=True)
class FeatureExcitationRow(FeatureValueMappingMixin):
    trajectory_id: str
    tier: str
    scenario_id: str
    true_class: str
    ...
    def feature_level(self, name: str) -> str:
        return self.feature_levels[name]

    @property
    def feature_level_names(self) -> tuple[str, ...]:
        return tuple(self.feature_levels)
...
@dataclass(frozen=True, slots=True)
class FeatureSeparationRow:
    feature: str
    mean_abs_cohens_d: float
    avg_pairwise_auc: float
    max_pairwise_auc: float
    min_pairwise_auc: float


@dataclass(frozen=True, slots=True)
class PairwiseMetricsRow:
    class_a: str
    class_b: str
    ...
    pairwise_classifier_accuracy: float
    average_log_likelihood_ratio: float
    pairwise_auc: float


def _flatten_feature_excitation_row(row: FeatureExcitationRow) -> dict[str, object]:
    flat = {
    ...
    flat.update(row.feature_values)
    flat.update({f"{feature_name}_level": level for feature_name, level in row.feature_levels.items()})
    return flat
...
def _numeric_features(row: FeatureRow, feature_names: tuple[str, ...]) -> dict[str, float]:
    ...
    for name in feature_names:
        value = row.feature_values.get(name, 0.0)
        numeric[name] = value if isinstance(value, (int, float)) else 0.0
    return numeric
...
def analyze_feature_datasets(
    *,
    ...
    excitation_rows: list[FeatureExcitationRow] = []
    for row in feature_rows_tuple:
        feature_values = {feature_name: row.feature_values.get(feature_name, 0.0) for feature_name in selected_feature_names}
        feature_levels = {
            feature_name: _excitation_level(
                float(feature_values[feature_name]) if isinstance(feature_values[feature_name], (int, float)) else 0.0,
    ...
    for class_name in class_names:
        ...
        for feature_name in selected_feature_names:
            values = [row.feature_values.get(feature_name, 0.0) for row in class_rows]
            sorted_values = sorted(values)
            summary_rows.append(
                FeatureSummaryRow(
    ...
    pairwise_rows: list[PairwiseMetricsRow] = []
    for index, class_a in enumerate(class_names):
        for class_b in class_names[index + 1 :]:
            pairwise_rows.append(PairwiseMetricsRow(**_pairwise_metrics(numeric_rows, selected_feature_names, class_a, class_b)))

    feature_separation_rows: list[FeatureSeparationRow] = []
    for feature_name in selected_feature_names:
        values_by_class = {
            class_name: [row.feature_values.get(feature_name, 0.0) for row in feature_rows_tuple if row.true_class == class_name]
            for class_name in class_names
        }
        pairwise_auc_values = []
        effect_sizes = []
    ...
    for row in excitation_rows:
        for feature_name in selected_feature_names:
            excitation_totals[feature_name][row.feature_levels[feature_name]] += 1
    ...
    return FeatureAnalysisResult(
        ...
        excitation_rows=tuple(_flatten_feature_excitation_row(row) for row in excitation_rows),
