...
def _status_color(status: str) -> str:
    return {
        ...
        "unsupported_class": "#ffc9c9",
        "unobserved_class": "#ffc9c9",
        "warning": "#ffe8cc",
        "blocker": "#ffc9c9",
    }.get(status, "#f8f9fa")
...
def render_static_feature_class_prior_audit_report(
    ...
    lines = [
        ...
        f"- Declared dimension: `{result.declared_dimension or 'generic_feature_vector'}`",
    ...
    lines.extend(
        [
            ...
            _markdown_table(
                tuple(
                    sorted(
                        result.class_pair_rows,
                        key=lambda row: (
                            {
                                "unsupported_class": 0,
                                "exact_feature_collision": 1,
                                "near_feature_collision": 2,
                                "distinct_feature_signature": 3,
                            }.get(str(row.get("collision_status", "")), 4),
                            float(row["pairwise_auc"]),
                        ),
                    )[:8]
                ),
                (
            ...
            "",
            _markdown_table(
                tuple(result.class_feature_signature_rows[:12]),
                (
                    "class_name",
                    "feature",
                    "sample_count",
            ...
            "",
            "## Class Observability And Collision",
            "",
            _markdown_table(
                tuple(result.class_observability_rows),
                (
                    "class_name",
                    "sample_count",
            ...
            "",
            _markdown_table(
                tuple(row for row in result.feature_alias_rows if row["alias_type"] != "distinct")[:12]
                or ({"feature_a": "none", "feature_b": "none", "alias_type": "none", "recommended_action": "keep"},),
                (
                    "feature_a",
                    "feature_b",
                    "alias_type",
