...
def render_kalman_observable_comparison_report(result: KalmanObservableComparisonResult) -> str:
    ...
    for row in result.rows:
        lines.append(
            f"| {row.observable_mode} | {row.overall_accuracy:.3f} | {row.endpoint_match_accuracy:.3f} | {row.short_accuracy:.3f} | {row.short_noisy_accuracy:.3f} | {row.outlier_accuracy:.3f} |"
        )
    lines.extend(
        [
            "",
