...
def render_pointwise_benchmark_report(result: PointwiseBenchmarkResult) -> str:
    ...
    lines = [
        ...
        "",
        "This milestone-1 benchmark classifies each measurement with a Gaussian class likelihood and updates the posterior sequentially across the trajectory.",
        "",
        "## Summary",
        "",
    ...
    for class_name, accuracy in summary.per_class_accuracy.items():
        lines.append(f"- `{class_name}`: {accuracy:.3f}")
    lines.extend(
        [
            "",
            "## Confusion Counts",
            "",
            "| True \\ Pred | " + " | ".join(summary.confusion_counts) + " |",
            "| --- | " + " | ".join("---" for _ in summary.confusion_counts) + " |",
        ]
    )
    for true_name in summary.confusion_counts:
        row = summary.confusion_counts[true_name]
        lines.append("| " + true_name + " | " + " | ".join(str(row[predicted]) for predicted in summary.confusion_counts) + " |")
    lines.extend(
        [
            "",
            "## Acceptance Notes",
...
def write_pointwise_benchmark_artifacts(
    ...
    *,
    ...
    _write_csv(confusion_matrix_path, _format_confusion_rows(benchmark_result.summary), ["true_class", *benchmark_result.summary.confusion_counts.keys()])
