...
from .common_experiment_harness import (
    ...
    _feature_set_scores_for_prefix,
    _feature_sigma,
    _gaussian_logpdf,
    _normalize_scores,
    _pair_priors,
...
def _render_report(
    *,
    ...
    return "\n".join(
        [
            "# Bayesian Evidence Walkthroughs",
            "",
            "This artifact grounds Bayesian posterior mechanics in the current common-study outputs rather than in a disconnected toy-only explainer.",
            "",
            "## Representative Sequential Walkthrough",
            "",
            f"- Study: `{selected_walkthrough['classifier_id']}_{selected_walkthrough['class_pair_id']}`",
            f"- Trajectory: `{selected_walkthrough['trajectory_id']}`",
            f"- True class: `{selected_walkthrough['true_class']}`",
            f"- Final prediction: `{selected_walkthrough['predicted_class']}` with confidence `{float(selected_walkthrough['confidence']):.3f}`",
            f"- Final log posterior odds `{final_row['class_a']}/{final_row['class_b']}`: `{float(final_row['log_posterior_odds']):.3f}`",
            "",
            "## Prior Sensitivity",
            "",
            f"- Empirical minimum prior on `{threshold_row['class_a']}` that keeps the final decision on `{threshold_row['class_a']}`: `{float(threshold_row['empirical_min_prior_a_for_class_a']):.3f}`",
            f"- Theoretical flip threshold for `{threshold_row['class_a']}` from the final likelihood ratio: `{float(threshold_row['theoretical_flip_threshold_prior_a']):.3f}`",
            "",
            "## Feature Evidence Example",
            "",
            f"- Study: `{feature_example['study_id']}`",
            f"- Trajectory: `{feature_example['trajectory_id']}`",
            f"- Dominant feature contribution: `{feature_example['top_feature']}`",
            f"- True-class posterior under feature-only scoring: `{float(feature_example['full_posterior_true']):.3f}`",
            "",
            "## Notes",
            "",
            ...
            "- `feature_contribution_examples.csv` uses per-feature Gaussian evidence differences and ablations to show why a simple pointwise study moved the way it did.",
        ]
    )
...
def write_bayesian_walkthrough_artifacts(
    ...
    *,
    ...
    _write_plot(_plot_prior_to_posterior_single_step(result), prior_to_posterior_single_step_path)
    _write_plot(_plot_likelihood_curves_with_feature_value(result), likelihood_curves_with_feature_value_path)
    _write_plot(_plot_posterior_timeline(result), posterior_timeline_path)
    _write_plot(_plot_log_odds_timeline(result), log_odds_timeline_path)
    _write_plot(_plot_bayes_factor_timeline(result), bayes_factor_timeline_path)
    _write_plot(_plot_prior_sensitivity_curve(result), prior_sensitivity_curve_path)
    _write_plot(_plot_feature_ablation_posterior(result), feature_ablation_posterior_path)
    _write_plot(_plot_confidence_threshold_crossing(result), confidence_threshold_crossing_path)
