...
from .corpus_adequacy_audit import CorpusAdequacyArtifacts, write_corpus_adequacy_artifacts
from .coverage_report import CoverageReportArtifacts, write_coverage_report_artifacts
...
def _render_index(
    *,
    ...
    for row in summary_rows:
        lines.append(
            f"| `{row['feature_set']}` | `{float(row['avg_pairwise_auc']):.3f}` | `{float(row['min_pairwise_auc']):.3f}` | `{float(row['avg_overlap']):.3f}` | `{float(row['max_overlap']):.3f}` | `{row['feature_set_status']}` | `{row['top_features']}` |"
        )
    lines.extend(
        [
            "",
            "## Hardest Class Boundaries",
            "",
    ...
    for row in class_pair_rows:
        lines.append(
            f"| `{row['class_pair']}` | `{float(row['pairwise_auc']):.3f}` | `{float(row['overlap_estimate']):.3f}` | `{float(row['mahalanobis_distance']):.3f}` | `{float(row['pairwise_classifier_accuracy']):.3f}` |"
        )
    lines.extend(
        [
            "",
            "## Named Feature Sets",
            "",
            "| Feature Set | Feature Analysis | PCA |",
            "| --- | --- | --- |",
        ]
    )
    subset_names = sorted(
    ...
    for feature_set_name in subset_names:
        ...
        lines.append(
            f"| `{feature_set_name}` | `{feature_artifacts.report_path.relative_to(feature_artifacts.run_dir.parent)}` | `{pca_artifacts.report_path.relative_to(pca_artifacts.run_dir.parent)}` |"
        )
    lines.extend(
        [
            "",
            "## Corpus Stress And Coverage",
            "",
            ...
            "## Rerun Contract",
            "",
            "Regenerate this bundle through the normal artifact exporter:",
            "",
            "```bash",
