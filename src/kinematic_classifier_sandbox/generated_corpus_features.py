...
def analyze_generated_corpus_features() -> GeneratedCorpusFeaturesResult:
    ...
    report_markdown = "\n".join(
        [
            "# Generated Corpus Feature Integration",
            "",
            "## Summary",
...
def write_generated_corpus_feature_artifacts(
    ...
    *,
    ...
    _write_csv(excitation_scores_path, list(payload.excitation_rows), list(payload.excitation_rows[0].keys()) if payload.excitation_rows else [])
    _write_csv(record_manifest_path, list(payload.record_rows), list(payload.record_rows[0].keys()) if payload.record_rows else [])
