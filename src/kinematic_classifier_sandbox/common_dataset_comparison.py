...
SCENARIO_TIMES = {
    "easy": tuple(float(step) for step in range(10)),
    "irregular": (0.0, 0.7, 1.6, 2.8, 4.1, 5.0, 6.6, 7.4, 8.9, 10.0),
    "endpoint_match": (0.0, 0.6, 1.4, 2.2, 3.7, 5.0),
    "short": (0.0, 0.5, 1.0, 1.5),
    "short_noisy": (0.0, 0.5, 1.0, 1.5),
    "outlier": tuple(float(step) for step in range(8)),
}
...
def _scenario_dynamics(scenario_name: str, true_class: str) -> tuple[float, float]:
...
def render_common_dataset_comparison_report(result: CommonComparisonResult) -> str:
    lines = [
        ...
        "",
        "This artifact evaluates the current technique families on the same shared binary dynamics corpus: constant velocity versus constant acceleration with easy, irregular-`dt`, matched-endpoint irregular, short-horizon boundary, short-horizon noisy, and outlier-corrupted scenarios.",
        "",
        "## Method Metrics",
        "",
        "| method | overall | easy | irregular | endpoint_match | short | short_noisy | outlier | prior_flip_fraction |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in result.rows:
        lines.append(
            f"| {row.method_name} | {row.overall_accuracy:.3f} | {row.easy_accuracy:.3f} | {row.irregular_accuracy:.3f} | {row.endpoint_match_accuracy:.3f} | {row.short_accuracy:.3f} | {row.noisy_accuracy:.3f} | {row.outlier_accuracy:.3f} | {row.prior_flip_fraction:.3f} |"
        )
    lines.extend(
        [
            "",
