...
def _render_walkthrough_markdown(walkthrough: IdentityPosteriorWalkthrough) -> str:
    ...
    lines = [
        f"# {walkthrough.title}",
        "",
        "This artifact illustrates the class-posterior update used in the identity speed benchmark for one concrete speed measurement step.",
        "",
        "## Measurement Model",
        "",
        ...
        "- Add a short-window within-class mode term so fast-pack bikes, near-limit horses, and pushed cars are not forced into one static cruise template.",
        ...
        "",
        "| Class | Prior | speed_shape | speed_validity | history_shape | mode_shape | dynamics_shape | total | Posterior |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for class_name in walkthrough.class_names:
        terms = walkthrough.log_terms[class_name]
        lines.append(
            f"| `{class_name}` | {walkthrough.prior_weights[class_name]:.3f} | {terms.get('speed_shape', 0.0):.3f} | {terms.get('speed_validity', 0.0):.3f} | {terms.get('history_shape', 0.0):.3f} | {terms.get('mode_shape', 0.0):.3f} | {terms.get('dynamics_shape', 0.0):.3f} | {terms.get('total', 0.0):.3f} | {walkthrough.posterior_weights[class_name]:.3f} |"
        )
    lines.extend(
        [
            "",
...
def render_identity_posterior_comparison_markdown(result: IdentityBenchmarkResult) -> str:
    ...
    lines = [
        "# Identity Posterior Comparison",
        "",
        "This artifact compares one successful identity posterior update against one confused update using the same score decomposition.",
        "",
        "## Cases",
        "",
        f"- Success case: `{success.run.scenario_name}` with true class `{success.run.expected_class}` and aggregate prediction `{success.run.aggregate_map_class}` at step `{success.step_index + 1}`",
        f"- Failure case: `{failure.run.scenario_name}` with true class `{failure.run.expected_class}` and aggregate prediction `{failure.run.aggregate_map_class}` at step `{failure.step_index + 1}`",
        "",
    ...
    for class_name in success.class_names:
        lines.append(
            f"| `{class_name}` | {success.prior_weights[class_name]:.3f} | {success.log_terms[class_name].get('total', 0.0):.3f} | {success.posterior_weights[class_name]:.3f} | {failure.prior_weights[class_name]:.3f} | {failure.log_terms[class_name].get('total', 0.0):.3f} | {failure.posterior_weights[class_name]:.3f} |"
        )
    ...
    for class_name in success.class_names:
        delta_total = success.log_terms[class_name].get("total", 0.0) - failure.log_terms[class_name].get("total", 0.0)
...
def render_identity_posterior_margin_trace_markdown(result: IdentityBenchmarkResult) -> str:
    ...
    lines = [
        "# Identity Posterior Margin Trace",
        "",
        ...
        "## Stepwise Margins",
        "",
        "| Step | z_t (mph) | True posterior | Predicted posterior | Posterior margin | Score margin | Dominant negative term |",
        "| ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    for step_index, step in enumerate(failure.run.steps):
        ...
        lines.append(
            f"| {step_index + 1} | {step.observed_speed_mph:.3f} | {step.updated_class_weights[true_class]:.3f} | {step.updated_class_weights[predicted_class]:.3f} | {posterior_margin:.3f} | {score_margin:.3f} | {dominant_negative_term} |"
        )
