...
from .sequential_bayes_accumulator import _logsumexp
...
def render_generic_classification_evidence_report(
    *,
    ...
    provider_lines = "\n".join(
        f"| {row['provider_id']} | {row['family']} | {row['evidence_category']} | {row['sensor_regime_id']} | {row['posterior_updater']} |"
        for row in evidence_provider_manifest
    )
    test_lines = "\n".join(
        f"| {row['test_id']} | {row['status']} |"
        for row in method_equivalence_tests
    )
    return "\n".join(
        [
            "# Classification Evidence Proof",
            "",
            "This artifact proves that current classifiers can be interpreted as evidence providers feeding a generic posterior updater rather than as isolated algorithm silos.",
            "",
            "## Evidence Providers",
            "",
