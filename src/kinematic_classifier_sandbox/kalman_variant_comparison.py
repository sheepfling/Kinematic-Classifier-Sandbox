...
def render_kalman_variant_comparison_report(result: KalmanVariantComparisonResult) -> str:
    lines = [
        "# Kalman Variant Comparison",
        "",
        "This artifact compares three Kalman-family variants on the shared dynamics corpus: plain, robust-measurement-only, and robust-plus-adaptive-process.",
        "",
    ...
    for row in result.rows:
        lines.append(
            f"| {row.variant_name} | {row.overall_accuracy:.3f} | {row.endpoint_match_accuracy:.3f} | {row.short_accuracy:.3f} | {row.short_noisy_accuracy:.3f} | {row.outlier_accuracy:.3f} |"
        )
    lines.extend(
        [
            "",
