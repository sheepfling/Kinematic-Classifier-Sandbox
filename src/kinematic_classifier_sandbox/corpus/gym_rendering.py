...
def render_corpus_gym_numeric_walkthrough_markdown(
    ...
    lines = [
        ...
        "",
        "```text",
        "U_gym = 0.22C + 0.14E + 0.14G + 0.14B + 0.14S + 0.12P - 0.10L - 0.14I",
        f"      = 0.22({reward.class_validity:.3f}) + 0.14({reward.feature_excitation:.3f}) + 0.14({reward.coverage_gain:.3f})",
        f"        + 0.14({reward.boundary_closeness:.3f}) + 0.14({reward.classifier_stress:.3f}) + 0.12({reward.prior_sensitivity:.3f})",
        f"        - 0.10({reward.leakage_penalty:.3f}) - 0.14({reward.physical_invalidity_penalty:.3f})",
