...
def _render_report(result: MonteCarloBenchmarkResult) -> str:
    ...
    lines = [
        "# Monte Carlo Accumulator Report",
        "",
        "This report aggregates time-indexed evidence from the sequential Bayesian accumulator.",
        "",
        "## Summary",
        "",
        ...
        "| metric | value |",
        "| --- | ---: |",
        f"| mean time to confidence | {summary.mean_time_to_confidence if summary.mean_time_to_confidence is not None else 'n/a'} |",
        f"| median time to confidence | {summary.median_time_to_confidence if summary.median_time_to_confidence is not None else 'n/a'} |",
        f"| mean time to correct classification | {summary.mean_time_to_correct_classification if summary.mean_time_to_correct_classification is not None else 'n/a'} |",
        f"| median time to correct classification | {summary.median_time_to_correct_classification if summary.median_time_to_correct_classification is not None else 'n/a'} |",
        "",
        "## Per-Class Summary",
        "",
    ...
    for class_name in class_names:
        row = summary.per_class[class_name]
        lines.append(
            f"| {class_name} | {row['precision']:.3f} | {row['recall']:.3f} | {row['f1']:.3f} | "
            f"{row['brier_score']:.3f} | {row['nll']:.3f} | {int(row['support'])} |"
        )
    lines.extend(
        [
            "",
    ...
    for row in result.calibration_bins:
        lines.append(
            f"| {int(row['bin_index'])} | {int(row['count'])} | {float(row['accuracy']):.3f} | {float(row['mean_confidence']):.3f} | {float(row['gap']):.3f} |"
        )
    lines.extend(
        [
            "",
...
def write_monte_carlo_artifacts(
    ...
    *,
    ...
    _write_csv(metrics_by_time_path, [dict(row) for row in benchmark_result.metrics_by_time], metrics_fieldnames)
    _write_csv(
    ...
    _write_csv(confusion_final_path, _confusion_to_rows(benchmark_result.final_confusion, class_names), ["true_class", *class_names, "unknown", "total"])
    _write_csv(
