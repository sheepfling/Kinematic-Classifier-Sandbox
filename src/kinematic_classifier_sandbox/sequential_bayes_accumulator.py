...
def _normalize_log_scores(log_scores: dict[str, float]) -> dict[str, float]:
    norm = _logsumexp(list(log_scores.values()))
...
def _render_report(result: AccumulatorBenchmarkResult) -> str:
    ...
    lines = [
        "# Sequential Bayesian Accumulator",
        "",
        "This benchmark accumulates class evidence sequentially in log space, supports configurable forgetting, and exposes prior sensitivity on an ambiguous track.",
        "",
        "## Summary",
        "",
    ...
    for class_name, value in summary.per_class_accuracy.items():
    ...
    for row in result.prior_sensitivity:
        lines.append(
            f"| {row['prior_a']:.2f} | {row['final_class']} | {row['final_confidence']:.3f} | {row['posterior_A']:.3f} | {row['posterior_B']:.3f} |"
        )
    lines.extend(
        [
            "",
...
def write_accumulator_artifacts(
    ...
    *,
    ...
    _write_csv(posterior_history_path, posterior_rows, posterior_fieldnames)
    _write_csv(log_odds_history_path, log_odds_rows, log_odds_fieldnames)
    _write_csv(confidence_crossings_path, confidence_rows, confidence_fieldnames)
    _write_csv(prior_sensitivity_path, [dict(row) for row in benchmark_result.prior_sensitivity], ["trajectory_id", "prior_a", "final_class", "final_confidence", "posterior_A", "posterior_B"])
