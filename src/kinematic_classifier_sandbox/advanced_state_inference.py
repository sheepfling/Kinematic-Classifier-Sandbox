...
def _write_yaml_like(path: Path, lines: Iterable[str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _logsumexp(values: Sequence[float]) -> float:
    pivot = max(values)
    return float(pivot + log(sum(exp(value - pivot) for value in values)))
...
def _normalize_log_scores(log_scores: dict[str, float]) -> dict[str, float]:
    ...
    return {name: float(exp(value - log_norm)) for name, value in log_scores.items()}
...
def _as_tuple(values: np.ndarray | Sequence[float]) -> tuple[float, ...]:
    array = np.asarray(values, dtype=float)
    return tuple(float(value) for value in array.tolist())


def _as_tuple_matrix(values: np.ndarray | Sequence[Sequence[float]]) -> tuple[tuple[float, ...], ...]:
    array = np.asarray(values, dtype=float)
    return tuple(tuple(float(value) for value in row.tolist()) for row in array)


def _block_diag(block: np.ndarray, repeats: int) -> np.ndarray:
...
def _gaussian_logpdf(residual: np.ndarray, covariance: np.ndarray) -> float:
    residual = _as_array(residual).reshape(-1, 1)
    covariance = np.asarray(covariance, dtype=float)
    covariance = 0.5 * (covariance + covariance.T)
    covariance += np.eye(covariance.shape[0], dtype=float) * 1e-9
    sign, logdet = np.linalg.slogdet(covariance)
    if sign <= 0:
        covariance += np.eye(covariance.shape[0], dtype=float) * 1e-6
        sign, logdet = np.linalg.slogdet(covariance)
    quadratic = float((residual.T @ np.linalg.solve(covariance, residual)).item())
    return float(-0.5 * (quadratic + logdet + covariance.shape[0] * log(2.0 * np.pi)))
...
def _kalman_update(
    ...
    identity = np.eye(predicted_covariance.shape[0], dtype=float)
...
class StateSpaceModeSpec:
    ...
    def process_bias(self) -> np.ndarray:
        block = np.array([0.0, 0.0, self.acceleration_bias], dtype=float)
        return np.tile(block, self.axes)

    def measurement_matrix(self) -> np.ndarray:
        matrix = np.zeros((self.axes, self.state_dim), dtype=float)
        for axis in range(self.axes):
            matrix[axis, axis * 3] = 1.0
        return matrix

    def measurement_covariance(self) -> np.ndarray:
        return np.eye(self.axes, dtype=float) * (self.measurement_sigma ** 2)

    def initial_mean(self, measurement: np.ndarray) -> np.ndarray:
        state = np.zeros(self.state_dim, dtype=float)
        observation = np.asarray(measurement, dtype=float).reshape(-1)
        position_values = observation if observation.size == self.axes else np.full(self.axes, float(observation[0]))
        for axis in range(self.axes):
            state[axis * 3] = float(position_values[axis])
        return state

    def initial_covariance(self, *, scale: float = 4.0) -> np.ndarray:
        covariance = np.zeros((self.state_dim, self.state_dim), dtype=float)
        for axis in range(self.axes):
            start = axis * 3
            covariance[start : start + 3, start : start + 3] = np.diag([scale, scale, scale])
        return covariance
...
def _initial_states(
    ...
    initial_measurement = np.array([witness.measurements[0]], dtype=float)
...
def run_imm_filter(
    ...
    *,
    ...
    for step_index, (time, measurement, true_mode, true_position, true_velocity, true_acceleration) in enumerate(
        ...
        mode_innovations: dict[str, tuple[float, ...]] = {}
        mode_innovation_variances: dict[str, tuple[tuple[float, ...], ...]] = {}
        updated_states: dict[str, tuple[np.ndarray, np.ndarray]] = {}
        for spec in mode_specs:
            mixed_mean = np.zeros(spec.state_dim, dtype=float)
            for source in mode_names:
                mixed_mean += mixing[spec.name][source] * states[source][0]
            mixed_covariance = np.zeros((spec.state_dim, spec.state_dim), dtype=float)
            for source in mode_names:
                source_weight = mixing[spec.name][source]
                source_mean, source_covariance = states[source]
                diff = source_mean - mixed_mean
                mixed_covariance += source_weight * (source_covariance + np.outer(diff, diff))
        ...
        combined_state_mean = np.zeros(mode_specs[0].state_dim, dtype=float)
        for spec in mode_specs:
            combined_state_mean += posterior[spec.name] * updated_states[spec.name][0]
        combined_state_covariance = np.zeros((mode_specs[0].state_dim, mode_specs[0].state_dim), dtype=float)
        for spec in mode_specs:
            mean_j, covariance_j = updated_states[spec.name]
            diff = mean_j - combined_state_mean
            combined_state_covariance += posterior[spec.name] * (covariance_j + np.outer(diff, diff))

        truth_vector = np.array([true_position, true_velocity, true_acceleration], dtype=float)
        state_rmse = float(np.sqrt(np.mean((combined_state_mean[:3] - truth_vector) ** 2)))
...
def render_advanced_state_inference_report(result: AdvancedStateInferenceResult) -> str:
    ...
    lines = [
        "# Advanced State Inference And IMM Lift",
        "",
        "This report documents the abstract advanced-filter contract and a 1D IMM witness backend that plugs into the current classifier ladder. The contract is intentionally generic so the same state/evidence/diagnostic surface can later lift to 3D PVA trajectories without changing the downstream evaluation path.",
        "",
        "## Summary",
        "",
        ...
        "## Contract",
        "",
        f"- State labels: `{', '.join(result.contract_result.contract.state_labels)}`",
        f"- Measurement labels: `{', '.join(result.contract_result.contract.measurement_labels)}`",
        f"- Interface methods: `{', '.join(result.contract_result.contract.interface_methods)}`",
        f"- Required outputs: `{', '.join(result.contract_result.contract.required_outputs)}`",
        f"- Diagnostics: `{', '.join(result.contract_result.contract.diagnostics)}`",
        "",
        "## Witness Interpretation",
        "",
        f"- Representative witness: `{representative.scenario_name}`",
        f"- Representative run: `{representative.trajectory_id}`",
        f"- Final IMM mode: `{representative.final_predicted_mode}`",
        f"- Final IMM class: `{representative.final_predicted_class}`",
        f"- Final confidence: `{representative.final_confidence:.3f}`",
        "",
        "## Transition Baseline",
        "",
        f"- Transition baseline post-switch accuracy: `{transition_summary.transition_post_switch_accuracy:.3f}`",
        f"- Kalman-bank reference remains the simpler model-based rung in the ladder; this IMM proof is the switching-aware extension.",
        "",
        "## 3D Lift Note",
        "",
...
def _render_mixing_probability_figure(result: AdvancedStateInferenceResult):
    ...
    representative = _representative_run(result)
    mode_names = list(representative.mode_names)
    matrix = np.zeros((len(mode_names), len(mode_names)), dtype=float)
    counts = np.zeros((len(mode_names), len(mode_names)), dtype=float)
    ...
    matrix = np.divide(matrix, np.maximum(counts, 1.0), out=np.zeros_like(matrix), where=counts > 0)
...
def _render_state_estimate_figure(result: AdvancedStateInferenceResult):
    ...
    estimated_sigma = [float(np.sqrt(max(step.combined_state_covariance[0][0], 0.0))) for step in representative.steps]
...
def write_advanced_state_inference_artifacts(
    ...
    *,
    ...
    mode_probability_rows: list[dict[str, object]] = []
    ...
    for run in analysis.runs:
        ...
        for step in run.steps:
            posterior_row = {
                ...
                **{f"posterior_{name}": step.mode_posteriors[name] for name in mode_names},
                **{f"class_posterior_{name}": step.class_posteriors[name] for name in class_names},
            }
            posterior_rows.append(posterior_row)
            mode_probability_rows.append(
                {
                    **posterior_row,
                    **{f"mode_prior_{name}": step.mode_prior[name] for name in mode_names},
                }
            )
                ...
                state_rows.append(
                    {
                        ...
                        "switch_event": step.switch_event,
                        "state_rmse": step.state_rmse,
                        **{f"state_mean_{index}": value for index, value in enumerate(step.state_mean_by_mode[target_mode])},
                        **{
                            f"state_cov_diag_{index}": step.state_covariance_by_mode[target_mode][index][index]
            ...
            state_rows.append(
                {
                    ...
                    "switch_event": step.switch_event,
                    "state_rmse": step.state_rmse,
                    **{f"state_mean_{index}": value for index, value in enumerate(step.combined_state_mean)},
                    **{
                        f"state_cov_diag_{index}": step.combined_state_covariance[index][index]
