...
class TechniqueComparisonArtifacts:
    ...
    metric_heatmap_svg_path: Path
    metric_heatmap_png_path: Path
    scatter_svg_path: Path
    scatter_png_path: Path
    capability_svg_path: Path
...
def analyze_technique_comparison(*, seed: int = 7) -> TechniqueComparisonResult:
    raw_row, robust_row = _windowed_rows(seed)
    rows = (
        _pointwise_row(seed),
        raw_row,
        robust_row,
        _accumulator_row(seed),
        _kalman_row(seed),
    )
    return TechniqueComparisonResult(rows=rows)
...
def render_technique_comparison_report(result: TechniqueComparisonResult) -> str:
    ...
    lines = [
        ...
        "## Summary Metrics",
        "",
        "| method | overall_accuracy | prior_flip_fraction | median_flip_threshold |",
        "| --- | ---: | ---: | ---: |",
    ]
    for row in result.rows:
        median_text = "n/a" if row.median_flip_threshold is None else f"{row.median_flip_threshold:.2f}"
        lines.append(
            f"| {row.method_name} | {row.overall_accuracy:.3f} | {row.prior_flip_fraction:.3f} | {median_text} |"
        )
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            ...
            "## Capability Flags",
            "",
            "| method | temporal_history | model_based | irregular_dt_native | outlier_aware |",
            "| --- | ---: | ---: | ---: | ---: |",
        ]
    )
    for row in result.rows:
        lines.append(
            f"| {row.method_name} | {row.uses_temporal_history:.1f} | {row.model_based:.1f} | {row.irregular_dt_native:.1f} | {row.outlier_aware:.1f} |"
        )
    lines.extend(
        [
            "",
...
def _render_capability_heatmap(result: TechniqueComparisonResult):
    plt = _prepare_matplotlib()
    capability_fields = ("uses_temporal_history", "model_based", "irregular_dt_native", "outlier_aware")
    ...
    ax.set_xticklabels(["temporal", "model", "irregular_dt", "outlier"], rotation=25, ha="right")
    ax.set_yticks(range(len(result.rows)))
    ax.set_yticklabels([row.method_name for row in result.rows])
    for row_index, row in enumerate(result.rows):
...
def write_technique_comparison_artifacts(
    ...
    *,
    ...
    metric_heatmap_svg_path = run_dir / "technique_metric_heatmap.svg"
    metric_heatmap_png_path = run_dir / "technique_metric_heatmap.png"
    scatter_svg_path = run_dir / "accuracy_vs_prior_fragility.svg"
    scatter_png_path = run_dir / "accuracy_vs_prior_fragility.png"
    capability_svg_path = run_dir / "technique_capability_heatmap.svg"
    ...
    _write_csv(
        ...
        ["method_name", "overall_accuracy", "prior_flip_fraction", "median_flip_threshold"],
    )
    _write_csv(
        scenario_csv_path,
        [
    ...
    metric_heatmap_svg_path.write_text(_figure_to_svg(_render_metric_heatmap(comparison)), encoding="utf-8")
    metric_heatmap_png_path.write_bytes(_figure_to_png(_render_metric_heatmap(comparison)))
    scatter_svg_path.write_text(_figure_to_svg(_render_accuracy_fragility_scatter(comparison)), encoding="utf-8")
    scatter_png_path.write_bytes(_figure_to_png(_render_accuracy_fragility_scatter(comparison)))
    capability_svg_path.write_text(_figure_to_svg(_render_capability_heatmap(comparison)), encoding="utf-8")
    ...
    return TechniqueComparisonArtifacts(
        ...
        metric_heatmap_svg_path=metric_heatmap_svg_path,
        metric_heatmap_png_path=metric_heatmap_png_path,
        scatter_svg_path=scatter_svg_path,
        scatter_png_path=scatter_png_path,
        capability_svg_path=capability_svg_path,
