...
def _effective_measurement_sigma(
    ...
    baseline_variance = (
        predicted_covariance[0][0] + base_measurement_sigma * base_measurement_sigma
    )
    normalized_innovation_squared = (innovation * innovation) / max(
        baseline_variance,
        1e-9,
    )
    gate = 9.0
...
def _transition_and_noise(
    ...
    return kalman_transition_and_noise(
        model.state_dim,
        model.process_sigma,
        dt,
        process_scale,
...
def _predict(
    ...
    predicted_covariance = _add_matrices(
    ...
    *,
    ...
    if prior is None:
        ...
        if normalization <= 1e-12:
            ...
            model_posterior = {
                name: value / normalization for name, value in model_posterior.items()
            }
    states = {}
    process_scales = {}
    for spec in model_specs:
    ...
    for step_index, (time, measurement) in enumerate(
        zip(trajectory.times, trajectory.measurements)
    ):
        ...
        for spec in model_specs:
            ...
            if derived_velocity_observation and spec.state_dim >= 2 and len(measurement_history_values) >= 3:
                ...
                slope_denominator = sum(
                    (obs_time - (sum(window_times) / max(len(window_times), 1))) ** 2
                    for obs_time in window_times
                )
                velocity_variance = max(
                    4.0 * (spec.measurement_sigma * spec.measurement_sigma)
                    / max(slope_denominator, 1e-6),
                    0.10,
                )
            ...
            if derived_acceleration_observation and len(measurement_history_values) >= 3:
                ...
                if spec.state_dim >= 3:
                    ...
                    acceleration_innovation_variance = acceleration_variance + max(
                        spec.process_sigma * spec.process_sigma,
                        0.05,
                    )
            ...
            if velocity_measurements is not None and velocity_measurement_sigma is not None:
                ...
                if spec.state_dim >= 2:
                    ...
                    velocity_sensor_innovation_variance = velocity_variance + max(
                        spec.process_sigma * spec.process_sigma,
                        0.05,
                    )
        ...
        class_log_scores = {
            class_name: _logsumexp(
                [
                    model_log_scores[spec.name]
                    for spec in model_specs
        ...
        for class_name in class_names:
            ...
            log_likelihood_terms[class_name] = _logsumexp(
                [model_log_likelihoods[spec.name] for spec in members]
            )
    ...
    final_states = {
        name: _pad_state(mean, covariance)
        for name, (mean, covariance) in states.items()
    }
...
def run_kalman_bank_benchmark(
    *,
    ...
    trajectories = generate_kalman_bank_trajectories(
        seed=seed,
...
def render_kalman_bank_report(result: KalmanBenchmarkResult) -> str:
    ...
    report.bullet_list(
        [
            f"{report.inline_code(class_name)}: {value:.3f}"
...
def _build_kalman_bank_figure(result: KalmanBenchmarkResult):
    ...
    colors = {
        spec.name: color
        for spec, color in zip(result.model_specs, ("#2563eb", "#16a34a", "#d97706"))
    }
    ...
    for row_axes, run in zip(axes, selected):
        ...
        for spec in result.model_specs:
            posterior_ax.plot(
                steps,
                [step.posterior_weights[spec.name] for step in run.steps],
    ...
    *,
    ...
    write_csv(
        ...
        [
            ...
            "step",
            "time",
            *[f"innovation_{name}" for name in class_names],
            *[f"innovation_variance_{name}" for name in class_names],
        ],
    )
    write_csv(
    ...
    write_csv(
        ...
        [
            {"true_class": true_name, **predicted_counts}
            for true_name, predicted_counts in benchmark.summary.confusion_counts.items()
        ],
