...
def _entropy(weights: dict[str, float]) -> float:
    return -sum(weight * log(max(weight, 1e-12)) for weight in weights.values())
...
def summarize_identity_runs(runs: tuple[IdentityClassificationRun, ...]) -> IdentityBenchmarkSummary:
    true_classes = sorted({run.expected_class for run in runs})
    ...
    scenario_confusion_counts = {
        run.family_name: {predicted_class: 0 for predicted_class in predicted_classes}
        for run in runs
    }
    ...
    transient_correct = 0
    terminal_correct = 0
    max_steps = max((len(run.steps) for run in runs), default=0)
    entropy_by_step: list[list[float]] = [[] for _ in range(max_steps)]
    mean_feature_probability_by_step = {
        feature_name: [[] for _ in range(max_steps)]
        for feature_name in FEATURE_NAMES
    }

    for run in runs:
        class_totals[run.expected_class] += 1
        confusion_counts[run.expected_class][run.aggregate_map_class] += 1
        transient_confusion_counts[run.expected_class][run.transient_map_class] += 1
        terminal_confusion_counts[run.expected_class][run.terminal_map_class] += 1
        scenario_confusion_counts[run.family_name][run.aggregate_map_class] += 1
        if run.aggregate_map_class == run.expected_class:
            correct += 1
        if run.transient_map_class == run.expected_class:
            transient_correct += 1
        if run.terminal_map_class == run.expected_class:
            terminal_correct += 1
        for feature_name in FEATURE_NAMES:
            true_present = feature_name in run.true_features
            detected_present = feature_name in run.detected_features
            ...
            if detected_present:
                class_feature_detection_counts[run.expected_class][feature_name] += 1
        ...
        for step_index, step in enumerate(run.steps):
            entropy_by_step[step_index].append(step.posterior_entropy)
            for feature_name in FEATURE_NAMES:
                mean_feature_probability_by_step[feature_name][step_index].append(step.feature_probabilities[feature_name])
    ...
    mean_feature_probability = {
        feature_name: tuple(
            ...
            for values in mean_feature_probability_by_step[feature_name]
        )
        for feature_name in FEATURE_NAMES
    }
    return IdentityBenchmarkSummary(
        total_runs=len(runs),
        overall_accuracy=(correct / len(runs)) if runs else 0.0,
        transient_accuracy=(transient_correct / len(runs)) if runs else 0.0,
        terminal_accuracy=(terminal_correct / len(runs)) if runs else 0.0,
...
def render_identity_benchmark_markdown(result: IdentityBenchmarkResult) -> str:
    ...
    lines = [
        "# 1D Identity Speed Benchmark",
        "",
        "This benchmark treats class as a static identity and updates posterior mass over `horse`, `car`, and `bike` as new speed measurements arrive.",
        "",
    ...
    for family_name in sorted(family_counts):
        lines.append(f"- {family_name}: {family_counts[family_name]} runs")
    lines.extend([
        "",
        "## Per-Class Accuracy",
        "",
    ])
    for class_name in sorted(result.summary.per_class_accuracy):
        lines.append(f"- {class_name}: {result.summary.per_class_accuracy[class_name]:.3f}")
    lines.extend(["", "## Class Confusion Counts", ""])
    ...
    for class_name in sorted(CLASS_FEATURE_ANCHORS):
        lines.append(f"- {class_name}: expected anchors -> {', '.join(CLASS_FEATURE_ANCHORS[class_name])}")
    ...
    for feature_name in FEATURE_NAMES:
        counts = result.summary.feature_confusion_counts[feature_name]
        lines.append(
            f"- {feature_name}: tp={counts['tp']}, fp={counts['fp']}, fn={counts['fn']}, tn={counts['tn']}"
        )
    lines.extend(["", "## True Feature vs Predicted Class Matrix", ""])
    for feature_name in FEATURE_NAMES:
        counts = ", ".join(
    ...
    lines.extend(["", "## Detected Feature vs Predicted Class Matrix", ""])
    for feature_name in FEATURE_NAMES:
        counts = ", ".join(
