...
from .common_experiment_harness import analyze_common_trajectory_corpus
from .corpus_classifier_scoring import analyze_corpus_classifier_scoring
from .corpus_objectives import default_corpus_objectives
from .generated_corpus_features import collect_generated_corpus_records
from .objective_driven_qd_archive import analyze_objective_driven_qd_archive
...
def write_selected_generated_corpus_artifacts(
    ...
    *,
    ...
    _write_csv(trajectories_path, list(payload.trajectory_rows), list(payload.trajectory_rows[0].keys()) if payload.trajectory_rows else [])
    _write_csv(observations_path, list(payload.observation_rows), list(payload.observation_rows[0].keys()) if payload.observation_rows else [])
    _write_csv(truth_states_path, list(payload.truth_state_rows), list(payload.truth_state_rows[0].keys()) if payload.truth_state_rows else [])
    _write_csv(events_path, list(payload.event_rows), list(payload.event_rows[0].keys()) if payload.event_rows else [])
    _write_csv(environment_traces_path, list(payload.environment_rows), list(payload.environment_rows[0].keys()) if payload.environment_rows else [])
    _write_csv(feature_matrix_path, list(payload.feature_rows), list(payload.feature_rows[0].keys()) if payload.feature_rows else [])
    _write_csv(class_validity_scores_path, list(payload.class_validity_rows), list(payload.class_validity_rows[0].keys()) if payload.class_validity_rows else [])
    _write_csv(classifier_scores_path, list(payload.classifier_score_rows), list(payload.classifier_score_rows[0].keys()) if payload.classifier_score_rows else [])
