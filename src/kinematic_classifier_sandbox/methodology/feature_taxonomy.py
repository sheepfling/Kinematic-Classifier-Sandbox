...
def render_generic_feature_taxonomy_report(
    *,
    ...
    report = MarkdownDocument("Generic Feature Taxonomy")
    report.paragraph(
        "This artifact proves that feature machinery is driven by generic metadata rather than "
        "only by named 1D feature bundles."
    )
    ...
    report.heading("Feature Taxonomy", level=2)
    report.table(
        ...
        [
            (
                ...
                row["geometry_assumption"],
                row["dimensional_transfer"],
            )
            for row in taxonomy_rows
        ],
    )
    report.heading("Feature Sets", level=2)
    report.table(
        ["feature_set_id", "history_behavior", "num_features", "roles"],
        [
            (row["feature_set_id"], row["history_behavior"], row["num_features"], ", ".join(row["roles"]))
            for row in feature_set_rows
        ],
    )
    report.heading("Tag Selection Examples", level=2)
    report.bullet_list(
        [
    ...
    report.bullet_list(
        [
            (
                "`geometry_assumption` distinguishes dimension-agnostic timing features from scalar-axis kinematic "
                "features that need a vector policy in 3D."
            ),
            (
                "`dimensional_transfer` makes the 3D lift constraint explicit per feature instead of "
                "hiding it in implementation details."
            ),
            (
                "`sensitivity_tags` and `dependency_tags` are queryable so feature bundles can be selected by "
                "semantics, not only by hardcoded names."
            ),
        ]
    )
    return report.text()
