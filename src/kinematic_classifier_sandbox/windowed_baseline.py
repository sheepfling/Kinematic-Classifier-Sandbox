...
def _render_report(result: WindowedBenchmarkResult) -> str:
    ...
    lines = [
        "# Windowed Feature Baseline",
        "",
        "This benchmark compares a raw-extrema classifier against a robust-extrema classifier built on sliding-window features.",
        "",
        "## Summary",
        "",
    ...
    for class_name in summary.per_class_accuracy_raw:
        lines.append(
            f"- `{class_name}` raw={summary.per_class_accuracy_raw[class_name]:.3f} robust={summary.per_class_accuracy_robust[class_name]:.3f}"
        )
    lines.extend(
        [
            "",
...
def write_windowed_benchmark_artifacts(
    ...
    *,
    ...
    _write_csv(confusion_raw_path, [{"true_class": true, **preds} for true, preds in benchmark_result.summary.confusion_raw.items()], ["true_class", *benchmark_result.summary.confusion_raw.keys()])
    _write_csv(confusion_robust_path, [{"true_class": true, **preds} for true, preds in benchmark_result.summary.confusion_robust.items()], ["true_class", *benchmark_result.summary.confusion_robust.keys()])
