...
def position_measurement_matrix() -> FloatArray:
    return array([[1.0, 0.0, 0.0]], dtype=float64)
...
def kalman_predict(
    ...
    *,
    ...
    bias = array([0.5 * acceleration_bias * dt * dt, acceleration_bias * dt, acceleration_bias], dtype=float64)
...
def kalman_update(
    ...
    s_inv = linalg.inv(innovation_covariance)
    gain = state.covariance @ h.T @ s_inv
    mean = state.mean.reshape(-1, 1) + gain @ innovation
    covariance = (eye(state.covariance.shape[0]) - gain @ h) @ state.covariance
