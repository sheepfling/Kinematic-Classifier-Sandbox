...
class ParticleFilterBank:
    def __init__(
        ...
        *,
        ...
        self.initial_log_prior = array(
            [log(prior_by_label[label]) for label in self.label_ids],
            dtype=float64,
        )
        self.state: ParticleFilterBankState | None = None

    def reset(self, trajectory_id: str, initial_particles_by_label: dict[str, ndarray]) -> None:
    ...
    def update(self, time: float, observation: ndarray) -> AdvancedFilterStep:
        ...
        best_index = int(argmax(posterior_values))
