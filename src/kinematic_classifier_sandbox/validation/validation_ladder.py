...
@dataclass(frozen=True, slots=True)
class ValidationLadderArtifacts:
    run_dir: Path
    schema_path: Path
    scores_path: Path
    decisions_path: Path
...
def analyze_validation_ladder(
    *,
    ...
    for study_id in canonical_studies:
        ...
        static_status = "fail" if not bool(static_row["compatible"]) else _status_from_score(static_score, pass_threshold=0.55, partial_threshold=0.40)
        levels.append(
            {
                "level_id": 1,
        ...
        if prediction_rows:
            overall_accuracy = sum(1.0 if row["predicted_class"] == row["true_class"] else 0.0 for row in prediction_rows) / len(prediction_rows)
            ...
            posterior_score = max(
                ...
                min(
                    1.0,
                    0.45 * (1.0 - calibration_gap) + 0.35 * (1.0 - confident_error_rate) + 0.20 * min(1.0, posterior_margin / 0.35),
                ),
            )
        ...
        partial_levels = {name for name, status in level_status_by_name.items() if status == "partial"}
        ...
        elif any(level_status_by_name[name] == "fail" for name in ("static_compatibility", "feature_separability", "oracle_separability", "classifier_performance")):
        ...
        elif any(level_status_by_name[name] == "fail" for name in ("corpus_adequacy", "posterior_and_calibration_quality", "prior_sensitivity", "stress_and_adversarial_robustness")):
            final_decision = "revise"
            rationale = "The study is viable but needs corpus, calibration, or robustness work before promotion."
        elif all(level_status_by_name[name] == "pass" for name in ("static_compatibility", "feature_separability", "oracle_separability", "classifier_performance", "posterior_and_calibration_quality", "prior_sensitivity")):
            final_decision = "promote"
    ...
    report_markdown = "\n".join(
        [
            "# Validation Ladder",
            "",
            "This artifact collapses the protocol, selected corpus, candidate generator, and common-study evidence into one ten-level ladder per canonical study.",
            "",
            "## Decision Counts",
            "",
        ...
        + [
            ...
            "- Corpus adequacy is pair-aware and uses the selected M19 corpus rather than a separate manual judgment.",
    ...
    return ValidationLadderResult(
        schema=protocol.validation_ladder_schema,
...
def write_validation_ladder_artifacts(
    ...
    *,
    ...
    if ladder.score_rows:
        _write_csv(scores_path, list(ladder.score_rows), list(ladder.score_rows[0].keys()))
    else:
        _write_csv(scores_path, [], ["study_id", "level_id", "level_name", "status", "score", "evidence_summary"])
    if ladder.decision_rows:
        _write_csv(decisions_path, list(ladder.decision_rows), list(ladder.decision_rows[0].keys()))
    else:
        _write_csv(decisions_path, [], ["study_id", "final_decision", "decision_rationale"])
    schema_path.write_text(json.dumps(ladder.schema, indent=2), encoding="utf-8")
    report_path.write_text(ladder.report_markdown, encoding="utf-8")
