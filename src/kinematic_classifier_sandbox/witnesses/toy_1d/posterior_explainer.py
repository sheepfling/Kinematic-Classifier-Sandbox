    ...
    for class_name in walkthrough.class_names:
        ...
        rows.append(
            (
                f"`{class_name}`",
                f"{prior:.3f}",
                f"{terms.get('dyn', 0.0):.3f}",
                f"{terms.get('speed', 0.0):.3f}",
                f"{terms.get('accel', 0.0):.3f}",
                f"{behavior_total:.3f}",
                f"{observed_total:.3f}",
                f"{mode_total:.3f}",
                f"{terms.get('total', 0.0):.3f}",
                f"{posterior:.3f}",
            )
        )
    report.table(
...
def render_posterior_comparison_markdown(result: ToyBenchmarkResult) -> str:
    ...
    report = MarkdownDocument("Toy 1D Posterior Comparison")
    report.paragraph(
    ...
    report.heading("Reading Guide", level=2)
    report.bullet_list(
        [
...
def render_posterior_margin_trace_markdown(result: ToyBenchmarkResult) -> str:
    ...
    report = MarkdownDocument("Toy 1D Posterior Margin Trace")
    report.paragraph(
    ...
    report.bullet_list(
        [
            ...
            "The trace is most useful for seeing whether the wrong class wins because of raw innovation fit, envelope terms, behavior terms, observed finite-difference terms, or within-class mode terms.",
        ]
    )
    return report.text()
