...
def render_irregular_window_report(result: WindowRegimeComparisonResult) -> str:
    lines = [
        ...
        "## Summary",
        "",
        "| window_mode | feature_name | mean_regular_irregular_gap | classification_accuracy | mean_window_duration_irregular | mean_window_sample_count_irregular |",
        "| --- | --- | ---: | ---: | ---: | ---: |",
    ]
    for row in result.summary_rows:
        lines.append(
            f"| {row.window_mode} | {row.feature_name} | {row.mean_regular_irregular_gap:.3f} | {row.classification_accuracy:.3f} | {row.mean_window_duration_irregular:.3f} | {row.mean_window_sample_count_irregular:.3f} |"
        )
    lines.extend(
        [
            "",
