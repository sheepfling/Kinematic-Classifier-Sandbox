...
class AdvancedFilterEvaluationTests(unittest.TestCase):
    ...
    def test_advanced_filter_comparison_outputs_decision_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("defer_to_gsf", decision_text)
            ...
            self.assertIn("gsf_preferred", artifacts.gsf_vs_pf_frontier_summary_path.read_text(encoding="utf-8"))
            self.assertIn("metric_split", artifacts.pf_vs_rbpf_frontier_summary_path.read_text(encoding="utf-8"))
            ...
            self.assertEqual(pf_row["status_level"], "witness_supported")
            rbpf_row = next(row for row in gate_rows if row["method_id"] == "rbpf_v1")
            self.assertEqual(rbpf_row["robustness_sweep_passes"], "not_yet")
            self.assertEqual(rbpf_row["status_level"], "witness_supported")
