...
class CorpusPolicySweepTests(unittest.TestCase):
    ...
    def test_sweep_outputs_nonempty_policy_results(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            artifacts = write_corpus_policy_tuning_artifacts(temp_dir)
            with artifacts.sweep_results_path.open(encoding="utf-8", newline="") as handle:
    ...
    def test_ablation_and_stability_outputs_have_expected_rows(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            artifacts = write_corpus_policy_tuning_artifacts(temp_dir)
            with artifacts.ablation_results_path.open(encoding="utf-8", newline="") as handle:
            ...
            with artifacts.stability_path.open(encoding="utf-8", newline="") as handle:
                jaccard = list(csv.DictReader(handle))
